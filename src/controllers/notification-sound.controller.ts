...
export class NotificationSoundController {
  ...
  static async getAllSounds(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const sounds = await NotificationSoundService.getAllSounds(
        activeOnly === "true"
      );
  ...
  static async batchUpdatePreferences(
    ...
    try {
      ...
      const results = await NotificationSoundService.batchUpdateUserPreferences(
        userId,
        preferences
      );
   ...
   */
  static async getSoundStatistics(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
